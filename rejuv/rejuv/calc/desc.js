...
function displayMove(gen, attacker, defender, move, damage, notation, field) {
    ...
    var minDisplay = toDisplay(notation, min, defender.maxHP());
    var maxDisplay = toDisplay(notation, max, defender.maxHP());
    var recoveryText = getRecovery(gen, attacker, defender, move, damage, field, notation).text;
...
function getRecovery(gen, attacker, defender, move, damage, field, notation) {
    if (notation === void 0) { notation = '%'; }
    var _a = __read((0, result_1.damageRange)(damage), 2), minDamage = _a[0], maxDamage = _a[1];
    ...
    if (attacker.hasItem('Shell Bell') && !ignoresShellBell) {
        ...
        else if (field.hasTerrain('Back-Alley')) {
            recovery[0] *= 0.77;
            recovery[1] *= 0.77;
        }
    }
    ...
    if (move.drain) {
        if (field.hasTerrain('Grassy') && move.named('Absorb', 'Mega Drain', 'Giga Drain', 'Horn Leach')) {
            move.drain = [2, 3];
        }
...
function getRecoil(gen, attacker, defender, move, damage, notation, field) {
    ...
    if (move.recoil) {
        ...
        if (!attacker.hasAbility('Rock Head') || !(field.hasTerrain('Electric') && move.named('Wild Charge'))) {
        ...
        if (move.named('Chloroblast') && field.hasTerrain('Forest') ||
            move.named('Steel Beam') && field.hasTerrain('Factory') ||
            move.named('Wave Crash') && field.hasTerrain('Water', 'Underwater')) {
            recoil = notation === '%' ? 24 : 25;
            text = '25% recoil damage';
        }
        else {
            recoil = notation === '%' ? 24 : 50;
...
function getKOChance(gen, attacker, defender, move, field, damage, err) {
    ...
    if (field.hasTerrain('Big-Top') &&
        (move.named('Brutal Swing', 'Strength', 'Wood Hammer', 'Dual Chop', 'Heat Crash', 'Sky Drop', 'Bulldoze', 'Icicle Crash', 'Body Slam', 'Stomp', 'Grav Apple', 'Giga Impact', 'Pound', 'Smack Down', 'Iron Tail', 'Meteor Mash', 'Crabhammer', 'Double Iron Bash', 'Dragon Rush', 'Bounce', 'Slam', 'Heavy Slam', 'Earthquake', 'Magnitude', 'High Horsepower', 'Ice Hammer', 'Dragon Hammer', 'Continental Crush', 'Stomping Tantrum') || (move.hasType('Fighting') && move.category === 'Physical'))) {
...
function getHazards(gen, defender, defenderSide, field) {
    ...
    if ((defenderSide.isSR && !defender.hasAbility('Magic Guard', 'Mountaineer')) ||
        (defenderSide.isSR && !defender.hasAbility('Magic Guard', 'Mountaineer')) && field.hasTerrain('Rocky', 'Wasteland', 'Inverse', 'Cave')) {
        var rockType = gen.types.get('rock');
        var effectiveness = rockType.effectiveness[defender.types[0]] *
            (defender.types[1] ? rockType.effectiveness[defender.types[1]] : 1);
        damage += Math.floor((effectiveness * defender.maxHP()) / 8);
        if (field.hasTerrain('Rocky', 'Wasteland', 'Cave')) {
            damage *= 2;
    ...
    else if ((defenderSide.isSR && !defender.hasAbility('Magic Guard', 'Mountaineer')) && field.hasTerrain('Infernal', 'Volcanic-Top', 'Dragon-Den', 'Corrupted')) {
        if (field.hasTerrain('Volcanic-Top', 'Dragon-Den', 'Infernal')) {
            ...
            var rockType = gen.types.get('poison');
            var effectiveness = rockType.effectiveness[defender.types[0]] *
                (defender.types[1] ? rockType.effectiveness[defender.types[1]] : 1);
            damage += Math.floor((effectiveness * defender.maxHP()) / 8);
            texts.push(damage + ' Stealth Rock (Poison) damage in ' + field.terrain);
        }
    }
    ...
    if (!defender.hasType('Flying') &&
        ...
        if (defenderSide.spikes === 1) {
            ...
            if (gen.num === 2) {
                ...
                texts.push(damage + ' 1 layer of Spikes');
            }
        }
        else if (defenderSide.spikes === 2) {
            damage += Math.floor(defender.maxHP() / 6);
            texts.push(damage + ' 2 layers of Spikes');
        }
        else if (defenderSide.spikes === 3) {
            damage += Math.floor(defender.maxHP() / 4);
            texts.push(damage + ' 3 layers of Spikes');
        }
    }
    else if (field.hasTerrain('Electric') && !defender.hasAbility('Magic Guard')) {
        var spikeType = gen.types.get('electric');
        var effectiveness = spikeType.effectiveness[defender.types[0]] *
            (defender.types[1] ? spikeType.effectiveness[defender.types[1]] : 1);
        if (defenderSide.spikes === 1) {
            damage += Math.floor((effectiveness * defender.maxHP()) / 8);
            texts.push(damage + ' 1 layer of Electric Spikes in ' + field.terrain);
        }
        else if (defenderSide.spikes === 2) {
            damage += Math.floor((effectiveness * defender.maxHP()) / 6);
            texts.push(damage + ' 2 layers of Electric Spikes in ' + field.terrain);
        }
        else if (defenderSide.spikes === 3) {
            damage += Math.floor((effectiveness * defender.maxHP()) / 6);
            texts.push(damage + ' 3 layers of Electric Spikes in ' + field.terrain);
        }
    }
    else if (field.hasTerrain('Wasteland') && (!(0, util_2.isGrounded)(defender, field) || !defender.hasAbility('Magic Guard'))) {
        if (defenderSide.spikes) {
            damage += Math.floor(defender.maxHP() * 0.33);
            texts.push('Consumed Spikes damage in ' + field.terrain);
        }
    }
    if (isNaN(damage)) {
        damage = 0;
...
function getEndOfTurn(gen, attacker, defender, move, field) {
    ...
    if (field.hasWeather('Sun', 'Harsh Sunshine')) {
        ...
        if (!defender.hasType('Shadow', 'Ghost', 'Dark') &&
            !defender.hasAbility('Magic Guard', 'Overcoat') &&
            !defender.hasItem('Safety Goggles')) {
            damage -= Math.floor(defender.maxHP() / 16);
            if (field.hasTerrain('Dimensional', 'Frozen')) {
                damage *= 2;
            }
            texts.push(damage + ' shadow sky damage');
        }
    }
    ...
    if (field.attackerSide.isSeeded && !attacker.hasAbility('Magic Guard')) {
        ...
        if (attacker.hasAbility('Liquid Ooze')) {
            damage -= recovery;
            if (field.hasTerrain('Wasteland', 'Murkwater', 'Corrupted')) {
                damage *= 2;
            }
    ...
    if (defender.hasStatus('psn')) {
        ...
        if (defender.hasAbility('Heatproof')) {
            if (field.hasTerrain('Icy')) {
                damage -= Math.floor(defender.maxHP() / (gen.num > 6 ? 64 : 32));
    ...
    switch (field.terrain) {
        case 'Bewitched':
            ...
            if ((0, util_2.isGrounded)(defender, field) && defender.hasType('Grass')) {
                damage += Math.floor(defender.maxHP() / 16);
            }
            if (damage > 0) {
                texts.push(damage + ' hp recovery in ' + field.terrain);
            }
            else {
                texts.push(damage + ' hp damage in ' + field.terrain);
            }
            break;
        case 'Corrosive':
            if (!defender.hasType('Poison', 'Steel')) {
                if ((0, util_2.isGrounded)(defender, field) && !defender.hasAbility('Toxic Boost', 'Poison Heal', 'Inmunity', 'Wonder Guard', 'Pastel Veil', 'Magic Guard')) {
                    var dmgType = gen.types.get('poison');
                    var effectiveness = dmgType.effectiveness[defender.types[0]] * (defender.types[1] ? dmgType.effectiveness[defender.types[1]] : 1);
                    if (effectiveness === 4) {
                        damage -= Math.floor(defender.maxHP() / 4);
                    }
                    else if (effectiveness === 2) {
                ...
                if (!defender.hasAbility('Wonder Guard')) {
                    damage -= Math.floor(defender.maxHP() / 16);
                }
                if (defender.hasAbility('Ingrain', 'Grass Pelt')) {
                    damage -= Math.floor(defender.maxHP() / 16);
                }
                if (defender.hasAbility('Poison Heal')) {
                    damage += Math.floor(defender.maxHP() / 16);
            ...
            break;
        case 'Corrosive-Mist':
            if (defender.hasAbility('Dry Skin') && !defender.hasType('Poison')) {
                damage -= Math.floor(defender.maxHP() / 16);
            }
            else if (defender.hasAbility('Dry Skin') && defender.hasType('Poison')) {
                damage += Math.floor(defender.maxHP() / 16);
            }
            if (defender.hasAbility('Poison Heal')) {
                damage += Math.floor(defender.maxHP() / 16);
            ...
            break;
        case 'Corrupted':
            if (defender.hasAbility('Grass Pelt', 'Leaf Guard', 'Flower Veil')) {
                damage -= Math.floor(defender.maxHP() / 16);
            }
            else if (defender.hasAbility('Poison Heal')) {
                damage += Math.floor(defender.maxHP() / 16);
            }
            else if (defender.hasAbility('Dry Skin')) {
                if (defender.hasType('Poison')) {
                    damage += Math.floor(defender.maxHP() / 16);
                }
                else {
            ...
            break;
        case 'Desert':
            if (defender.hasType('Grass', 'Water') && field.hasWeather('Sun', 'Harsh Sunshine')) {
                damage -= Math.floor(defender.maxHP() / 8);
            }
            if (defender.hasAbility('Dry Skin')) {
                damage -= Math.floor(defender.maxHP() / 16);
        ...
        case 'Short-Circuit-2':
            ...
            texts.push(damage + ' recovery in ' + field.terrain);
            break;
        case 'Forest':
        ...
        case 'Grassy':
            ...
            break;
        case 'Haunted':
            if (defender.hasStatus('slp') && !defender.hasType('Ghost')) {
                damage -= Math.floor(defender.maxHP() / 16);
                texts.push(damage + ' hp damage in ' + field.terrain);
            }
            break;
        ...
        case 'Misty':
            ...
            break;
        case 'Murkwater':
            if (!defender.hasType('Poison', 'Steel') && (0, util_2.isGrounded)(defender, field) && !defender.hasAbility('Inmunity', 'Toxic Boost', 'Poison Heal', 'Magic Guard', 'Wonder Guard', 'Pastel Veil', 'Surge Surfer')) {
                var dmgType = gen.types.get('poison');
                var effectiveness = dmgType.effectiveness[defender.types[0]] * (defender.types[1] ? dmgType.effectiveness[defender.types[1]] : 1);
                if (effectiveness === 4) {
                    damage -= Math.floor(defender.maxHP() / 4);
                ...
                if (defender.hasAbility('Dry Skin', 'Water Absorb', 'Magma Armor', 'Flame Body')) {
                    damage *= 2;
                }
            }
            if (defender.hasAbility('Dry Skin', 'Water Absorb') && (0, util_2.isGrounded)(defender, field) && defender.hasType('Poison')) {
                damage += Math.floor(defender.maxHP() / 16);
            }
            if (defender.hasAbility('Poison Heal') && (0, util_2.isGrounded)(defender, field)) {
                damage += Math.floor(defender.maxHP() / 16);
            }
            if (damage > 0) {
                texts.push(damage + 'hp recovery in ' + field.terrain);
        ...
        case 'Rainbow':
            ...
            break;
        case 'Rocky':
            if ((move.named('Fake Out') || (move.named('Fling') && attacker.hasItem('King\'s Rock', 'Razor Fang'))) && !defender.hasAbility('Sturdy', 'Steadfast')) {
                damage -= Math.floor(defender.maxHP() / 4);
            }
            texts.push(damage + ' flinch damage in ' + field.terrain);
            break;
        case 'Snowy-Mountain':
            if (defender.hasAbility('Ice Body')) {
                damage += Math.floor(defender.maxHP() / 16);
        ...
        case 'Swamp':
            ...
            break;
        case 'Underwater':
            if (!defender.hasAbility('Swift Swim', 'Steelworker', 'Magic Guard')) {
                var dmgType = gen.types.get('water');
                var effectiveness = dmgType.effectiveness[defender.types[0]] * (defender.types[1] ? dmgType.effectiveness[defender.types[1]] : 1);
                if (effectiveness === 4) {
                    damage -= Math.floor(defender.maxHP() / 4);
                }
                ...
                if (defender.hasAbility('Magma Armor', 'Flame Body')) {
                    damage *= 2;
                }
                if (defender.hasAbility('Dry Skin', 'Water Absorb')) {
                    damage += Math.floor(defender.maxHP() / 64);
                }
                if (damage > 0) {
                    texts.push(damage + ' hp recovery in ' + field.terrain);
            ...
            break;
        case 'Volcanic':
            if ((0, util_2.isGrounded)(defender, field) && !defender.hasType('Fire') && !defender.hasAbility('Magma Armor', 'Flame Body', 'Flash Fire', 'Flare Boost', 'Water Veil', 'Heat Proof', 'Water Bubble')) {
                var dmgType = gen.types.get('fire');
                var effectiveness = dmgType.effectiveness[defender.types[0]] * (defender.types[1] ? dmgType.effectiveness[defender.types[1]] : 1);
                if (effectiveness === 4) {
                    damage -= Math.floor(defender.maxHP() / 4);
                }
                ...
                if (defender.hasAbility('Ice Body', 'Leaf Guard', 'Grass Pelt', 'Fluffy')) {
                    damage *= 2;
                }
                texts.push(damage + ' hp damage in ' + field.terrain);
            }
            break;
        case 'Volcanic-Top':
            if (move.named('Bulldoze', 'Magnitude', 'Eruption', 'Fever Pitch', 'Lava Plume', 'Magma Drift', 'Earth Power', 'Precipice Blades') ||
                (defender.hasAbility('Desolate Land') || attacker.hasAbility('Desolate Land')) &&
                    !defender.hasAbility('Magma Armor', 'Flash Fire', 'Flare Boost', 'Blaze', 'Flame Body', 'Solid Rock', 'Sturdy', 'Battle Armor', 'Shell Armor', 'Water Bubble', 'Magic Guard', 'Wonder Guard', 'Prism Armor')) {
                var dmgType = gen.types.get('fire');
                var effectiveness = dmgType.effectiveness[defender.types[0]] * (defender.types[1] ? dmgType.effectiveness[defender.types[1]] : 1);
                if (effectiveness > 1) {
                    damage -= Math.floor(defender.maxHP() / 4);
